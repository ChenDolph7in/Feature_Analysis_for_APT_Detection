    ...
    with open(filename, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if proto == '6':
                ...
                print(port_client+direction+port_server)
                ...
                if key not in f_dic['tcp'].keys():
                    f_dic['tcp'][key] = {}
                    f_dic['tcp'][key][port_client+'->'+port_server] = []
                    f_dic['tcp'][key][port_server+'->'+port_client] = []
                    f_dic['tcp'][key]['<->'] = []
                    f_dic['tcp'][key][port_client+direction+port_server].append(ip_len)
                    f_dic['tcp'][key]['time'] = {}
                    f_dic['tcp'][key]['time'][port_client+direction+port_server] = [timestamp, 0]
                    f_dic['tcp'][key]['time']['<->'] = [timestamp, 0]
                    # new

                    f_dic['tcp'][key]['tcp_flag'] = [port_client+direction+port_server+":"+tcp_flags]
                    ...
                    if port_client+direction+port_server not in f_dic['tcp'][key]['time'].keys():
                        f_dic['tcp'][key]['time'][port_client+direction+port_server] = [timestamp, 0]
                    f_dic['tcp'][key][port_client+direction+port_server].append(ip_len)
                    f_dic['tcp'][key]['time'][port_client+direction+port_server][1] = timestamp - f_dic['tcp'][key]['time'][port_client+direction+port_server][0]
                f_dic['tcp'][key]['<->'].append(ip_len)
                f_dic['tcp'][key]['time']['<->'][1] = timestamp - f_dic['tcp'][key]['time']['<->'][0]
                # new
                f_dic['tcp'][key]['tcp_flag'].append(port_client+direction+port_server+":"+tcp_flags)
                ...
                if key not in f_dic['udp'].keys():
                    f_dic['udp'][key] = {}
                    f_dic['udp'][key][port_client+'->'+port_server] = []
                    f_dic['udp'][key][port_client+'->'+port_server] = []
                    f_dic['udp'][key]['<->'] = []
                    f_dic['udp'][key][port_client+direction+port_server].append(ip_len)
                    f_dic['udp'][key]['time'] = {}
                    f_dic['udp'][key]['time'][port_client+direction+port_server] = [timestamp, 0]
                    f_dic['udp'][key]['time']['<->'] = [timestamp, 0]
                else:
                    if port_client+direction+port_server not in f_dic['udp'][key]['time'].keys():
                        f_dic['udp'][key]['time'][port_client+direction+port_server] = [timestamp, 0]
                    f_dic['udp'][key][port_client+direction+port_server].append(ip_len)
                    f_dic['udp'][key]['time'][port_client+direction+port_server][1] = timestamp - f_dic['udp'][key]['time'][port_client+direction+port_server][0]
...
def read_batch():
    ...
    new_name = ['tcp_flag','tcp_ack', 'more_seg_flag' ,'tls_version' ,'certs', 'sni']
